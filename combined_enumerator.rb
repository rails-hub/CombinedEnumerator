...
class CombinedOrderedEnumerator < Enumerator
  ...
  # overwrite the enumerator take function to set the limit var
  def take(num)
    @limit = num
    super
  end
    ...
    super() do |yielder|
      ...
      enums.inject([]) do |n, e|
        n + Enumerator.new do |y|
          raise UnorderedEnumerator.new(e) unless e.take(limit).each_cons(2).all? { |a, b| (a <=> b) != 1 } #raises unordered enumerator exception if enumerator isnt in ascending order

          e.each { |v|
            y.yield v
          }
        end.take(limit)
      end.sort.each { |v| yielder.yield v }
    end
  end
